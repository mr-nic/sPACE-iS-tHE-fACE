...
function getMorphTime() {
  ...
  return s ? +s.value / 10 : 0;
}

function startMorph(targetState) {
  const dur = getMorphTime();
  if (dur <= 0) { applyTargetState(targetState); return; }
  morphStartState = {};
...
function genRandomSliderVal(id) {
  const s = sl(id); if (!s) return undefined;
...
window.resetSection = function(name) {
  ...
  if (morphTime > 0) {
    const target = {};
    if (sec.sliders) sec.sliders.forEach(id => { const s=sl(id); if(s&&s.dataset.default!==undefined) target[id]=+s.dataset.default; });
    if (sec.trackballs) { target.trackballs={}; sec.trackballs.forEach(k=>{target.trackballs[k]={x:0,y:0};}); }
...
window.homeSection = function(name) {
  ...
  morphStartState.colour=colourPicker.value;
  morphStartState.trackballs={};
  Object.keys(trackballState).forEach(k=>{morphStartState.trackballs[k]={x:trackballState[k].x,y:trackballState[k].y};});
  morphTargets=target; morphDuration=dur*1000; morphStart=performance.now();
};
...
window.randomSection = function(name) {
  ...
  if (morphTime > 0) {
    ...
    if (sec.colour) target.colour='#'+Math.floor(Math.random()*16777215).toString(16).padStart(6,'0');
    if (sec.trackballs) { target.trackballs={}; sec.trackballs.forEach(k=>{target.trackballs[k]={x:Math.random()*2-1,y:Math.random()*2-1};}); }
    startMorph(target);
  } else {
    if (sec.sliders) sec.sliders.forEach(id=>randomSlider(id));
    if (sec.colour) { const rh='#'+Math.floor(Math.random()*16777215).toString(16).padStart(6,'0'); colourPicker.value=rh; colourValEl.textContent=rh; }
    if (sec.trackballs) sec.trackballs.forEach(k=>randomTrackball(k));
  }
  if (sec.selects) sec.selects.forEach(id=>{const el=sl(id);if(el)el.selectedIndex=Math.floor(Math.random()*el.options.length);});
};

window.resetAll = function() { Object.keys(SECTIONS).forEach(k=>{if(!isLocked(k))window.resetSection(k);}); };
...
window.randomAll = function() {
  ...
  if (morphTime > 0) {
    ...
    Object.keys(SECTIONS).forEach(k=>{
      if(isLocked(k))return; const sec=SECTIONS[k];
      if(sec.sliders)sec.sliders.forEach(id=>{target[id]=genRandomSliderVal(id);});
      if(sec.colour)target.colour='#'+Math.floor(Math.random()*16777215).toString(16).padStart(6,'0');
      if(sec.trackballs)sec.trackballs.forEach(tb=>{target.trackballs[tb]={x:Math.random()*2-1,y:Math.random()*2-1};});
    });
    startMorph(target);
    Object.keys(SECTIONS).forEach(k=>{
      if(isLocked(k))return; const sec=SECTIONS[k];
...
window.gentleResetAll = function() {
  ...
  Object.keys(SECTIONS).forEach(k=>{
    if(isLocked(k))return; const sec=SECTIONS[k];
    if(sec.sliders)sec.sliders.forEach(id=>{const s=sl(id);if(s&&s.dataset.default!==undefined)target[id]=+s.dataset.default;});
    if(sec.trackballs)sec.trackballs.forEach(tb=>{target.trackballs[tb]={x:0,y:0};});
  });
...
sl("videoFileInput").onchange=e=>{
  ...
  video.onloadeddata=()=>{
    video.play(); videoFileActive=true; lastTimestamp=-1;
  ...
  video.onerror=()=>{statusEl.textContent="Video load error";videoFileActive=false;};
  e.target.value='';
};
...
function updateVidPlayPauseState(){
  ...
  if(video.paused){playBtn.style.borderColor="#0a0";playBtn.style.color="#0c0";pauseBtn.style.borderColor="";pauseBtn.style.color="";}
  else{playBtn.style.borderColor="";playBtn.style.color="";pauseBtn.style.borderColor="#f90";pauseBtn.style.color="#fa0";}
}

window.vidPlay=function(){if(!videoFileActive)return;video.play();lastTimestamp=-1;updateVidPlayPauseState();};
window.vidPause=function(){if(!videoFileActive)return;video.pause();updateVidPlayPauseState();};
window.vidStop=function(){if(!videoFileActive)return;video.pause();video.currentTime=0;lastTimestamp=-1;updateVidPlayPauseState();};

const vidSeekEl=sl("vidSeek");
if(vidSeekEl){
  let vidSeeking=false;
  vidSeekEl.onmousedown=vidSeekEl.ontouchstart=()=>{vidSeeking=true;};
  vidSeekEl.onmouseup=vidSeekEl.ontouchend=()=>{vidSeeking=false;if(videoFileActive&&video.duration){video.currentTime=(vidSeekEl.value/1000)*video.duration;lastTimestamp=-1;}};
  vidSeekEl.oninput=()=>{if(vidSeeking&&videoFileActive&&video.duration){video.currentTime=(vidSeekEl.value/1000)*video.duration;lastTimestamp=-1;}};
}

function updateVideoTransportUI(){
  if(!videoFileActive)return;
  const seekEl=sl("vidSeek"),timeEl=sl("vidTime");
  if(seekEl&&video.duration&&!isNaN(video.duration))seekEl.value=Math.round((video.currentTime/video.duration)*1000);
...
if(webcamBtnEl){
  webcamBtnEl.onclick=async()=>{
    ...
    video.onloadeddata=null; videoFileActive=false; lastTimestamp=-1;
    const vc=sl("vidControls");if(vc)vc.style.display="none";
    webcamBtnEl.style.display="none";
    try{await startWebcam();statusEl.textContent="Tracking…";}
    catch(err){statusEl.textContent="Webcam error: "+err.message;}
  };
}
...
let audioBufWritePos=0,audioScrollOffset=0,audioFreqData=null;
...
async function startMic(){
  ...
  audioWaveform=new Uint8Array(analyser.fftSize);audioFreqData=new Uint8Array(analyser.frequencyBinCount);
  audioBuf.fill(0);audioBufWritePos=0;audioScrollOffset=0;
  const btn=sl("micBtn");if(btn){btn.textContent="🎤 Mic ON";btn.style.borderColor="#0a0";btn.style.color="#0c0";}
}

function stopMic(){
  if(micStream){micStream.getTracks().forEach(t=>t.stop());micStream=null;}
  analyser=null;audioWaveform=null;audioFreqData=null;audioBuf.fill(0);audioLine.fill(0);audioBands.fill(0);
  const btn=sl("micBtn");if(btn){btn.textContent="🎤 Mic off";btn.style.borderColor="";btn.style.color="";}
}

window.toggleMic=function(){if(micStream)stopMic();else startMic().catch(e=>console.error("Mic error:",e));};

// ─── Audio file playback ──────────────────────────────────────────
let audioTrackEl=null,audioTrackMediaSource=null,audioTrackAnalyser=null;
let audioTrackPlaying=false,audioTrackDuration=0;

sl("audioFileBtn").onclick=()=>sl("audioFileInput").click();
sl("audioPlayBtn").onclick=()=>toggleAudioTrack();
sl("audioStopBtn").onclick=()=>stopAudioTrack();

sl("audioFileInput").onchange=async e=>{
  const f=e.target.files[0];if(!f)return;
  if(!audioCtx)audioCtx=new(window.AudioContext||window.webkitAudioContext)();
  if(audioCtx.state==="suspended")await audioCtx.resume();
  if(audioTrackEl){audioTrackEl.pause();audioTrackEl.src="";}
  if(audioTrackMediaSource){audioTrackMediaSource.disconnect();}
  audioTrackEl=new Audio();audioTrackEl.src=URL.createObjectURL(f);audioTrackEl.preload="auto";
  audioTrackMediaSource=audioCtx.createMediaElementSource(audioTrackEl);
  if(!audioTrackAnalyser){audioTrackAnalyser=audioCtx.createAnalyser();audioTrackAnalyser.fftSize=2048;audioTrackAnalyser.smoothingTimeConstant=0.3;}
  audioTrackMediaSource.connect(audioTrackAnalyser);audioTrackAnalyser.connect(audioCtx.destination);
  if(sl("audioTrackToAnalyser")&&sl("audioTrackToAnalyser").checked){
  ...
  audioTrackEl.onloadedmetadata=()=>{
    audioTrackDuration=audioTrackEl.duration;
    const nameEl=sl("audioTrackName");if(nameEl)nameEl.textContent=f.name;
    const timeEl=sl("audioTrackTime");if(timeEl)timeEl.textContent=`0:00 / ${fmtTime(audioTrackDuration)}`;
    const seekEl=sl("audioTrackSeek");if(seekEl)seekEl.value=0;
  };
  audioTrackEl.onended=()=>{audioTrackPlaying=false;const btn=sl("audioPlayBtn");if(btn)btn.textContent="▶";};
  e.target.value='';
};
...
function toggleAudioTrack(){
  if(!audioTrackEl||!audioTrackEl.src)return;
  if(audioCtx&&audioCtx.state==="suspended")audioCtx.resume();
  if(audioTrackPlaying){
    audioTrackEl.pause();audioTrackPlaying=false;const btn=sl("audioPlayBtn");if(btn)btn.textContent="▶";
    logTimecodeEvent("audio_pause",{at:audioTrackEl.currentTime.toFixed(2)});
  }else{
    audioTrackEl.play();audioTrackPlaying=true;const btn=sl("audioPlayBtn");if(btn)btn.textContent="❚❚";
    logTimecodeEvent("audio_play",{offset:audioTrackEl.currentTime.toFixed(2)});
  }
}

function stopAudioTrack(){
  if(!audioTrackEl)return;
  audioTrackEl.pause();audioTrackEl.currentTime=0;audioTrackPlaying=false;
  const btn=sl("audioPlayBtn");if(btn)btn.textContent="▶";
  const seekEl=sl("audioTrackSeek");if(seekEl)seekEl.value=0;
  const timeEl=sl("audioTrackTime");if(timeEl&&audioTrackDuration)timeEl.textContent=`0:00 / ${fmtTime(audioTrackDuration)}`;
  logTimecodeEvent("audio_stop");
}

sl("audioTrackToAnalyser").onchange=function(){
  if(this.checked&&audioTrackAnalyser){
    analyser=audioTrackAnalyser;
    if(!audioWaveform)audioWaveform=new Uint8Array(analyser.fftSize);
    if(!audioFreqData)audioFreqData=new Uint8Array(analyser.frequencyBinCount);
  }
...
sl("audioTrackSeek").oninput=function(){if(!audioTrackEl||!audioTrackDuration)return;audioTrackEl.currentTime=(+this.value/1000)*audioTrackDuration;};

function updateAudioTrackUI(){
  if(!audioTrackEl||!audioTrackPlaying)return;
  const pos=audioTrackEl.currentTime;
  const seekEl=sl("audioTrackSeek");if(seekEl&&audioTrackDuration)seekEl.value=Math.round((pos/audioTrackDuration)*1000);
  const timeEl=sl("audioTrackTime");if(timeEl)timeEl.textContent=`${fmtTime(pos)} / ${fmtTime(audioTrackDuration)}`;
}
...
function updateAudioWaveform(){
  ...
  if(mode==="waveform"){
    ...
    for(let i=0;i<audioWaveform.length;i++){audioBuf[audioBufWritePos]=((audioWaveform[i]-128)/128)*gain;audioBufWritePos=(audioBufWritePos+1)%AUDIO_BUF_LEN;}
    if(scrollSpeed!==0){audioScrollOffset+=scrollSpeed*0.3;if(audioScrollOffset>AUDIO_BUF_LEN)audioScrollOffset-=AUDIO_BUF_LEN;if(audioScrollOffset<0)audioScrollOffset+=AUDIO_BUF_LEN;}
    const readStart=scrollSpeed===0?(audioBufWritePos-LINE_SAMPLES+AUDIO_BUF_LEN)%AUDIO_BUF_LEN:Math.floor(audioScrollOffset)%AUDIO_BUF_LEN;
    for(let col=0;col<LINE_SAMPLES;col++)audioLine[col]=audioBuf[(readStart+col)%AUDIO_BUF_LEN];
  }else{
    analyser.getByteFrequencyData(audioFreqData);
    const binCount=audioFreqData.length;
    for(let i=0;i<NUM_LINES;i++){
      const bandStart=Math.floor((i/NUM_LINES)*binCount),bandEnd=Math.floor(((i+1)/NUM_LINES)*binCount);
      let sum=0,count=0;for(let b=bandStart;b<bandEnd;b++){sum+=audioFreqData[b];count++;}
...
function buildGrid(landmarks){
  ...
  if((subAmt>0||pullAmt>0)&&landmarks){
    ...
    for(let i=0;i<landmarks.length;i++){
      ...
      const left=Math.max(0,Math.floor(lmx-radius)),right=Math.min(W-1,Math.ceil(lmx+radius));
      for(let x=left;x<=right;x++){const dist=Math.abs(x-lmx)/radius;eyeMouthXMap[x]=Math.max(eyeMouthXMap[x],Math.max(0,1-dist));}
    }
  }
  ...
  for(let row=0;row<NUM_LINES;row++){
    ...
    if(audioMix>0){
      if(audioMode==="waveform"){
        for(let col=0;col<LINE_SAMPLES;col++){if(!faceLock||line[col]>0.5){line[col]+=audioLine[col]*audioMix*maxD*0.4;if(line[col]<0)line[col]=0;}}
      }else{
        const bandVal=audioBands[row]||0;
        if(bandVal>0){for(let col=0;col<LINE_SAMPLES;col++){if(!faceLock||line[col]>0.5)line[col]+=bandVal*audioMix*maxD*0.5;}}
      }
    }
    grid.push(line);
...
function drawLines(grid,safeAmt,safeFlip){
  ...
  const safeL=hasSafe?W*safeAmt/100:0,safeR=hasSafe?W-W*safeAmt/100:W;
  const safeT=hasSafe?H*safeAmt/100:0,safeB=hasSafe?H-H*safeAmt/100:H;

  for(let row=0;row<NUM_LINES;row++){
    const baseY=tY+row*gap;
    const rowCut=cutMask?cutMask[row]:null;
    const rawPts=[];
    for(let c=0;c<LINE_SAMPLES;c++){rawPts.push({x:c*sw,y:baseY-grid[row][c],cut:!!(rowCut&&rowCut[c]>=cutThreshold)});}

    let pts;
    if(hasSafe&&safeFlip){
      pts=[];
      for(let i=0;i<rawPts.length;i++){
        const rp=rawPts[i];let x=rp.x,y=rp.y;
        if(x<safeL){const over=safeL-x;x=safeL;y=y+over;}else if(x>safeR){const over=x-safeR;x=safeR;y=y+over;}
        if(y<safeT){const over=safeT-y;y=safeT;const cx=(safeL+safeR)/2;x=x<cx?x-over:x+over;x=Math.max(safeL,Math.min(safeR,x));}
        if(y>safeB)y=safeB;
    ...
    clearGlow(ctx);
    ctx.beginPath();
    const occL=hasSafe?safeL:0,occR=hasSafe?safeR:W,occB=hasSafe?safeB:H;
    ctx.moveTo(occL,baseY);
    for(let i=0;i<pts.length;i++){
      let px=pts[i].x,py=pts[i].y;
      if(hasSafe&&!safeFlip){px=Math.max(safeL,Math.min(safeR,px));py=Math.max(safeT,Math.min(safeB,py));}
      ctx.lineTo(px,py);
    }
    ctx.lineTo(occR,baseY);ctx.lineTo(occR,occB);ctx.lineTo(occL,occB);
    ctx.closePath();ctx.fillStyle="#000";ctx.fill();

    applyGlow(ctx);ctx.strokeStyle=colour;ctx.lineWidth=thick;
    let inSegment=false;ctx.beginPath();
    for(let i=0;i<pts.length;i++){
      const p=pts[i];
      if(p.cut){if(inSegment){ctx.stroke();ctx.beginPath();inSegment=false;}continue;}
      if(hasSafe&&!safeFlip){if(p.x<safeL||p.x>safeR||p.y<safeT||p.y>safeB){if(inSegment){ctx.stroke();ctx.beginPath();inSegment=false;}continue;}}
      if(!inSegment){ctx.moveTo(p.x,p.y);inSegment=true;}else{ctx.lineTo(p.x,p.y);}
    }
    if(inSegment){ctx.lineTo(hasSafe?safeR:W,baseY);ctx.stroke();}
    ctx.beginPath();clearGlow(ctx);
  }
}
...
function drawGhostFeatures(lm){
  ...
  if(eyeOp>0){applyGlow(ctx);ctx.strokeStyle=getColour(eyeOp);ctx.lineWidth=1.2;[LEFT_EYE_CONTOUR,RIGHT_EYE_CONTOUR].forEach(cont=>{ctx.beginPath();cont.forEach((idx,j)=>{if(idx>=lm.length)return;const p=lm[idx];j===0?ctx.moveTo(p.x*W,p.y*H):ctx.lineTo(p.x*W,p.y*H);});ctx.closePath();ctx.stroke();ctx.fillStyle=getColour(eyeOp*0.3);ctx.fill();});clearGlow(ctx);}
...
function drawVerticalLines(lm){
  ...
  function dv(ids,op){if(op<=0)return;const b=bbox(ids),px=(b.Mx-b.mx)*0.1,l=(b.mx-px)*W,r=(b.Mx+px)*W,t=b.my*H,bt=b.My*H,st=(r-l)/(dens+1);applyGlow(ctx);ctx.strokeStyle=getColour(op);ctx.lineWidth=0.8;for(let i=1;i<=dens;i++){const x=l+i*st;ctx.beginPath();ctx.moveTo(x,t);ctx.lineTo(x,bt);ctx.stroke();}clearGlow(ctx);}
...
function drawPixelStretch(){
  ...
  const rawH=1.0-Math.abs(dir.y),rawV=1.0-Math.abs(dir.x);
  const hAmt=Math.max(0,rawH)*amt,vAmt=Math.max(0,rawV)*amt;
  if(hAmt<=0&&vAmt<=0)return;
  if(stretchWork.width!==cW||stretchWork.height!==cH){stretchWork.width=cW;stretchWork.height=cH;}
  stretchWorkCtx.clearRect(0,0,cW,cH);stretchWorkCtx.imageSmoothingEnabled=false;
  for(let d=0;d<density;d++){
    let linePos;
    if(density===1){linePos=pos;}else{const spread=(d/(density-1)-0.5);const explodeSpread=spread*(1+explode*4);linePos=pos+explodeSpread*0.5;if(linePos<0||linePos>1)continue;}
    if(hAmt>0.01){const srcRow=Math.max(0,Math.min(cH-1,Math.floor(linePos*cH)));const smearH=Math.floor(cH*hAmt/density);const smearTop=Math.floor(linePos*cH-smearH/2);stretchWorkCtx.drawImage(camWork,0,srcRow,cW,1,0,smearTop,cW,smearH);}
    if(vAmt>0.01){const srcCol=Math.max(0,Math.min(cW-1,Math.floor(linePos*cW)));const smearW=Math.floor(cW*vAmt/density);const smearLeft=Math.floor(linePos*cW-smearW/2);stretchWorkCtx.drawImage(camWork,srcCol,0,1,cH,smearLeft,0,smearW,cH);}
  }
  if(strContrast!==1||hueSpeed!==0){
    const imgData=stretchWorkCtx.getImageData(0,0,cW,cH);const sd=imgData.data;
    const hueRad=stretchHueOffset*Math.PI/180,cosH=Math.cos(hueRad),sinH=Math.sin(hueRad);
    for(let i=0;i<sd.length;i+=4){
      if(sd[i+3]===0)continue;
      let r=sd[i],g=sd[i+1],b=sd[i+2];
      if(strContrast!==1){r=Math.max(0,Math.min(255,((r-128)*strContrast)+128));g=Math.max(0,Math.min(255,((g-128)*strContrast)+128));b=Math.max(0,Math.min(255,((b-128)*strContrast)+128));}
      if(hueSpeed!==0){const nr=r*(.213+.787*cosH-.213*sinH)+g*(.715-.715*cosH-.715*sinH)+b*(.072-.072*cosH+.928*sinH);const ng=r*(.213-.213*cosH+.143*sinH)+g*(.715+.285*cosH+.140*sinH)+b*(.072-.072*cosH-.283*sinH);const nb=r*(.213-.213*cosH-.787*sinH)+g*(.715-.715*cosH+.715*sinH)+b*(.072+.928*cosH+.072*sinH);r=Math.max(0,Math.min(255,Math.round(nr)));g=Math.max(0,Math.min(255,Math.round(ng)));b=Math.max(0,Math.min(255,Math.round(nb)));}
      sd[i]=r;sd[i+1]=g;sd[i+2]=b;
    }
    stretchWorkCtx.putImageData(imgData,0,0);
  }
  if(blend!=="normal"){
    const stretchData=stretchWorkCtx.getImageData(0,0,cW,cH);const feedData=camWorkCtx.getImageData(0,0,cW,cH);const sd=stretchData.data,fd=feedData.data;
    for(let i=0;i<sd.length;i+=4){
      if(sd[i+3]===0)continue;
      const sr=sd[i],sg=sd[i+1],sb=sd[i+2],fr=fd[i],fg=fd[i+1],fb=fd[i+2];
      if(blend==="feedCuts"){const feedLuma=(fr*0.299+fg*0.587+fb*0.114)/255;sd[i+3]=Math.round(sd[i+3]*(1-feedLuma));}
      else if(blend==="stretchCuts"){sd[i]=Math.max(0,fr-sr);sd[i+1]=Math.max(0,fg-sg);sd[i+2]=Math.max(0,fb-sb);sd[i+3]=255;}
      else if(blend==="xor"){sd[i]=Math.round(Math.abs(sr-fr));sd[i+1]=Math.round(Math.abs(sg-fg));sd[i+2]=Math.round(Math.abs(sb-fb));const diff=Math.abs((sr*0.299+sg*0.587+sb*0.114)-(fr*0.299+fg*0.587+fb*0.114))/255;sd[i+3]=Math.round(diff*255);}
      else if(blend==="mask"){const feedLuma=(fr*0.299+fg*0.587+fb*0.114)/255;sd[i+3]=Math.round(sd[i+3]*(1-feedLuma));}
      else if(blend==="burn"){sd[i]=fr>0?Math.max(0,255-Math.floor((255-sr)*255/fr)):0;sd[i+1]=fg>0?Math.max(0,255-Math.floor((255-sg)*255/fg)):0;sd[i+2]=fb>0?Math.max(0,255-Math.floor((255-sb)*255/fb)):0;sd[i+3]=255;}
    }
  ...
  const baseDx=(W-baseDw)/2+camOff.x*W*0.5,baseDy=(H-baseDh)/2+camOff.y*H*0.5;
  const cx=baseDx+baseDw/2,cy=baseDy+baseDh/2;
  const dw=baseDw*strScale,dh=baseDh*strScale;
  const dx=cx-dw/2,dy=cy-dh/2;
  ctx.save();ctx.globalAlpha=opacity;ctx.imageSmoothingEnabled=false;ctx.drawImage(stretchWork,dx,dy,dw,dh);ctx.imageSmoothingEnabled=true;ctx.globalAlpha=1;ctx.restore();
}
...
function drawWebcamOverlay(){
  const op=sl("camMixSlider").value/100;if(op<=0)return;
  const W=canvas.width,H=canvas.height;
  ...
  let workW,workH;
  if(pixelSize>1){workW=Math.max(4,Math.floor(W/pixelSize));workH=Math.max(4,Math.floor(H/pixelSize));}
  else{workW=Math.floor(W*0.5);workH=Math.floor(H*0.5);}
  if(camWork.width!==workW||camWork.height!==workH){camWork.width=workW;camWork.height=workH;}
  camWorkCtx.save();camWorkCtx.translate(workW,0);camWorkCtx.scale(-1,1);camWorkCtx.drawImage(video,0,0,workW,workH);camWorkCtx.restore();
  if(locked&&lastGrid){
    const srcData=camWorkCtx.getImageData(0,0,workW,workH);const dst=camWorkCtx.createImageData(workW,workH);
    const sd=srcData.data,dd=dst.data;const m=workH*0.04,tY=m,bY=workH-m;const gridScaleY=(bY-tY)/(NUM_LINES-1);
    for(let y=0;y<workH;y++){
      const gridY=(y-tY)/gridScaleY;
      if(gridY<0||gridY>=NUM_LINES-1){for(let x=0;x<workW;x++){const i=(y*workW+x)*4;dd[i]=sd[i];dd[i+1]=sd[i+1];dd[i+2]=sd[i+2];dd[i+3]=sd[i+3];}continue;}
      const rowLow=Math.floor(gridY),rowHigh=Math.min(rowLow+1,NUM_LINES-1),frac=gridY-rowLow;
      for(let x=0;x<workW;x++){
        ...
        const si=(srcY*workW+x)*4,di=(y*workW+x)*4;dd[di]=sd[si];dd[di+1]=sd[si+1];dd[di+2]=sd[si+2];dd[di+3]=sd[si+3];
      }
    }
    camWorkCtx.putImageData(dst,0,0);
  }
  const imgData=camWorkCtx.getImageData(0,0,workW,workH);const d=imgData.data;
  for(let i=0;i<d.length;i+=4){
    let r=d[i],g=d[i+1],b=d[i+2];
    r=Math.max(0,Math.min(255,((r-128)*contrast)+128));g=Math.max(0,Math.min(255,((g-128)*contrast)+128));b=Math.max(0,Math.min(255,((b-128)*contrast)+128));
    const luma=(r*0.299+g*0.587+b*0.114)/255;
    if(mode==="mono"){const grey=Math.round(r*0.299+g*0.587+b*0.114);const h=colourPicker.value;const cr=parseInt(h.slice(1,3),16)/255,cg=parseInt(h.slice(3,5),16)/255,cb=parseInt(h.slice(5,7),16)/255;r=Math.round(grey*cr);g=Math.round(grey*cg);b=Math.round(grey*cb);}
    else if(mode==="pxl2000"){let grey=Math.round(r*0.299+g*0.587+b*0.114);grey=Math.max(0,Math.min(255,((grey-128)*2.5)+128));const noise=(Math.random()-0.5)*40;grey=Math.max(0,Math.min(255,grey+noise));r=Math.min(255,Math.round(grey*1.05));g=Math.round(grey*0.95);b=Math.round(grey*0.85);}
    else if(mode==="1bit"){const grey=r*0.299+g*0.587+b*0.114;const v=grey>128?255:0;r=v;g=v;b=v;}
    if(lumaKey>0){let alpha;if(lumaInvert){alpha=luma>lumaKey?0:Math.min(255,Math.round(((lumaKey-luma)/(lumaKey+0.001))*255));}else{alpha=luma<lumaKey?0:Math.min(255,Math.round(((luma-lumaKey)/(1-lumaKey+0.001))*255));}d[i+3]=alpha;}
  ...
  ctx.save();ctx.globalAlpha=op;ctx.imageSmoothingEnabled=(pixelSize<=1);
  if(camBlur>0)ctx.filter=`blur(${camBlur}px)`;
  ctx.drawImage(camWork,dx,dy,dw,dh);ctx.filter="none";ctx.globalAlpha=1;ctx.imageSmoothingEnabled=true;ctx.restore();
}
...
function getFormatGuideRect() {
  const fmt = sl('formatPreset') ? sl('formatPreset').value : 'free';
  if (fmt === 'free') return null;
  ...
  if (canvasRatio > ratio) {
    ...
    gH = H; gW = H * ratio; gX = (W - gW) / 2; gY = 0;
  } else {
    // Canvas taller than target — letterbox
    gW = W; gH = W / ratio; gX = 0; gY = (H - gH) / 2;
  }
  return { x: gX, y: gY, w: gW, h: gH };
}

function drawFormatGuide() {
  const rect = getFormatGuideRect();
  if (!rect) return;
  ctx.save();
...
window.applyFormatPreset = function() { updateFormatDims(); };

function updateFormatDims() {
  const el = sl('formatDims');
  if (!el) return;
  const rect = getFormatGuideRect();
  if (!rect) { el.textContent = ''; return; }
  const fmt = sl('formatPreset').value;
  el.textContent = fmt + ' — ' + Math.round(rect.w) + '×' + Math.round(rect.h) + 'px';
}

function getFormatLabel() {
  const fmt = sl('formatPreset') ? sl('formatPreset').value : 'free';
  if (fmt === 'free') return 'free';
  const rect = getFormatGuideRect();
  if (!rect) return fmt;
  return fmt + ' (' + Math.round(rect.w) + 'x' + Math.round(rect.h) + ')';
}
...
function updateFxState(){
  ...
  if(cycleSpeed>0)colourCycleHue=(colourCycleHue+cycleSpeed*0.5)%360;
  applyChaos();
}

// ─── Chaos engine ─────────────────────────────────────────────────
let chaosBaseState=null,chaosFrameCount=0;
const CHAOS_SLIDERS=["blurSlider","dispSlider","powSlider","thickSlider","eyeOpSlider","mouthOpSlider","ghostSubSlider","ghostPullSlider","eyeVertSlider","mouthVertSlider","vertDensSlider","pcAllSlider","pcEyeSlider","pcMouthSlider","pcOutlineSlider","pcSizeSlider","glowSlider","glowSpreadSlider","ghostScaleSlider","vertScaleSlider","pcScaleSlider","ghostRotSlider","vertRotSlider","pcRotSlider","camPixelSlider","stretchOpSlider","stretchAmtSlider","stretchExplodeSlider"];
...
function applyChaos(){
  ...
  if(chaosAmt<=0){
    if(chaosBaseState){
      CHAOS_SLIDERS.forEach(id=>{const s=sl(id);if(s&&chaosBaseState[id]!==undefined){s.value=chaosBaseState[id];if(s.oninput)s.oninput();}});
      if(chaosBaseState._trackballs){Object.keys(chaosBaseState._trackballs).forEach(k=>{if(trackballState[k]){trackballState[k]={...chaosBaseState._trackballs[k]};const d=sl("dot"+k.charAt(0).toUpperCase()+k.slice(1));if(d){d.style.left=((trackballState[k].x+1)/2*100)+"%";d.style.top=((trackballState[k].y+1)/2*100)+"%";}}});}
      chaosBaseState=null;
    }
  ...
  if(chaosLevel!==chaosLastLoggedLevel){chaosLastLoggedLevel=chaosLevel;logTimecodeEvent("chaos_level",{chaos:chaosLevel*10});}
  if(!chaosBaseState){
    chaosBaseState={};CHAOS_SLIDERS.forEach(id=>{const s=sl(id);if(s)chaosBaseState[id]=+s.value;});
    chaosBaseState._trackballs={};Object.keys(trackballState).forEach(k=>{chaosBaseState._trackballs[k]={x:trackballState[k].x,y:trackballState[k].y};});
  }
  if(chaosAmt>0){
    const jitterAmt=chaosAmt*0.3;
    CHAOS_SLIDERS.forEach(id=>{const s=sl(id);if(!s||chaosBaseState[id]===undefined)return;const range=+s.max - +s.min;const jitter=(Math.random()-0.5)*range*jitterAmt;const newVal=Math.max(+s.min,Math.min(+s.max,chaosBaseState[id]+jitter));s.value=Math.round(newVal);if(s.oninput)s.oninput();});
  }
  if(chaosAmt>0.3){
    const intensity=(chaosAmt-0.3)/0.3;
    if(Math.random()<intensity*0.3){const h=Math.floor(Math.random()*360),s2=100,l2=50+Math.random()*20;const c=(1-Math.abs(2*l2/100-1))*s2/100,x=c*(1-Math.abs((h/60)%2-1)),m=l2/100-c/2;let r1=0,g1=0,b1=0;if(h<60){r1=c;g1=x;}else if(h<120){r1=x;g1=c;}else if(h<180){g1=c;b1=x;}else if(h<240){g1=x;b1=c;}else if(h<300){r1=x;b1=c;}else{r1=c;b1=x;}const toHex=v=>Math.round((v+m)*255).toString(16).padStart(2,'0');colourPicker.value="#"+toHex(r1)+toHex(g1)+toHex(b1);}
    const dispS=sl("dispSlider");if(dispS)dispS.value=Math.min(+dispS.max,Math.round((chaosBaseState.dispSlider||86)+intensity*150));
    const glowS=sl("glowSlider");if(glowS)glowS.value=Math.min(+glowS.max,Math.round(intensity*80));
  }
  if(chaosAmt>0.6){
    const intensity=(chaosAmt-0.6)/0.4;
    ...
    Object.keys(trackballState).forEach(k=>{if(Math.random()<intensity*0.1){trackballState[k].x=Math.max(-1,Math.min(1,trackballState[k].x+(Math.random()-0.5)*intensity*0.4));trackballState[k].y=Math.max(-1,Math.min(1,trackballState[k].y+(Math.random()-0.5)*intensity*0.4));const d=sl("dot"+k.charAt(0).toUpperCase()+k.slice(1));if(d){d.style.left=((trackballState[k].x+1)/2*100)+"%";d.style.top=((trackballState[k].y+1)/2*100)+"%";}}});
  }
  if(chaosAmt>0.9&&chaosFrameCount%3===0){CHAOS_SLIDERS.forEach(id=>{const s=sl(id);if(!s)return;s.value=Math.round(+s.min+Math.random()*(+s.max - +s.min));if(s.oninput)s.oninput();});fxExplode=Math.random();fxPulse=1;}
}
...
function render(lm,grid){
  ...
  if(fxExplode>0||fxImplode>0){
    modGrid=[];const centreRow=NUM_LINES/2,centreSample=LINE_SAMPLES/2;
    for(let row=0;row<NUM_LINES;row++){
      const newLine=new Float32Array(LINE_SAMPLES);
      for(let col=0;col<LINE_SAMPLES;col++){
        const dy=(row-centreRow)/centreRow,dx=(col-centreSample)/centreSample;
  ...
  if(postZoom){ctx.save();ctx.translate(W/2,H/2);ctx.scale(zoom,zoom);ctx.translate(-W/2,-H/2);}

  const visZoom=zoom!==1&&!zoomAll;
  if(visZoom){ctx.save();ctx.translate(W/2,H/2);ctx.scale(zoom,zoom);ctx.translate(-W/2,-H/2);}

  ctx.save();ctx.translate(W,0);ctx.scale(-1,1);

  if(pulseThickBoost>0){const s=sl("thickSlider");const orig=+s.value;s.value=orig+pulseThickBoost*10;drawLines(modGrid,safeAmt,safeFlip);s.value=orig;}
  else{drawLines(modGrid,safeAmt,safeFlip);}

  drawGhostFeatures(lm);drawVerticalLines(lm);drawPointCloud(lm);
  ctx.restore();

  if(visZoom)ctx.restore();

  drawWebcamOverlay();drawPixelStretch();

  if(postZoom)ctx.restore();

  // Strobe flash
  if(fxStrobe&&fxStrobeFrame%4<2){ctx.fillStyle="#fff";ctx.fillRect(0,0,W,H);}

  // Invert
  if(fxInvert){ctx.globalCompositeOperation="difference";ctx.fillStyle="#fff";ctx.fillRect(0,0,W,H);ctx.globalCompositeOperation="source-over";}
  ...
  if(trailAmt>0){
    ...
    trailCtx.globalAlpha=1-trailAmt;trailCtx.drawImage(canvas,0,0);
    if(!trailOutside&&safeAmt>0){const sx=W*safeAmt/100,sy=H*safeAmt/100;ctx.save();ctx.beginPath();ctx.rect(sx,sy,W-sx*2,H-sy*2);ctx.clip();ctx.drawImage(trailCanvas,0,0);ctx.restore();}
    else{ctx.drawImage(trailCanvas,0,0);}
    trailCtx.globalAlpha=1;trailCtx.clearRect(0,0,W,H);trailCtx.drawImage(canvas,0,0);
  }
  ...
  // Format guide — drawn on top of everything
  drawFormatGuide();
...
let timecodeLog=[],recordingStartTime=0,isRecording=false;
...
function exportTimecodesSRT(){
  ...
  const fmt=getFormatLabel();
  const rect=getFormatGuideRect();
  const dimStr=rect?`${Math.round(rect.w)}x${Math.round(rect.h)}`:'full canvas';
  // First entry: metadata
  let srt='1\n00:00:00,000 --> 00:00:00,500\n';
  srt+=`FORMAT: ${fmt} | Canvas: ${canvas.width}x${canvas.height}\n\n`;
  timecodeLog.forEach((ev,i)=>{
    const startMs=ev.time,endMs=i<timecodeLog.length-1?timecodeLog[i+1].time:startMs+2000;
    srt+=(i+2)+'\n';
...
function exportTimecodesCSV(){
  ...
  const rect=getFormatGuideRect();
  const dimStr=rect?`${Math.round(rect.w)}x${Math.round(rect.h)}`:'full canvas';
  let csv='# sPACE iS tHE faCE — Timecode Export\n';
  csv+=`# Format: ${fmt}\n`;
  csv+=`# Canvas: ${canvas.width}x${canvas.height}\n`;
  csv+=`# Crop dimensions: ${dimStr}\n`;
  csv+='timecode,seconds,type,detail\n';
  timecodeLog.forEach(ev=>{
    const sec=(ev.time/1000).toFixed(3),tc=msToSRT(ev.time);
    let detail='';
...
canvas.addEventListener("touchend",e=>{
  const now=Date.now();
  if(now-lastTapTime<300){e.preventDefault();if(!document.fullscreenElement){(document.documentElement.requestFullscreen||document.documentElement.webkitRequestFullscreen||function(){}).call(document.documentElement);}else{(document.exitFullscreen||document.webkitExitFullscreen||function(){}).call(document);}}
  lastTapTime=now;
},{passive:false});
canvas.addEventListener("dblclick",()=>{if(!document.fullscreenElement){(document.documentElement.requestFullscreen||document.documentElement.webkitRequestFullscreen||function(){}).call(document.documentElement);}else{(document.exitFullscreen||document.webkitExitFullscreen||function(){}).call(document);}});

function pickRecordingFormat(){
  if(isMobile){const mobileOpts=[{mimeType:"video/webm;codecs=vp8",ext:"webm",type:"video/webm"},{mimeType:"video/webm",ext:"webm",type:"video/webm"}];for(const o of mobileOpts){if(MediaRecorder.isTypeSupported(o.mimeType))return o;}return mobileOpts[mobileOpts.length-1];}
  const options=[{mimeType:"video/mp4;codecs=avc1.42E01E",ext:"mp4",type:"video/mp4"},{mimeType:"video/webm;codecs=h264",ext:"webm",type:"video/webm"},{mimeType:"video/webm;codecs=vp9",ext:"webm",type:"video/webm"},{mimeType:"video/webm;codecs=vp8",ext:"webm",type:"video/webm"},{mimeType:"video/webm",ext:"webm",type:"video/webm"}];
...
recBtn.onclick=()=>{
  if(mediaRecorder&&mediaRecorder.state==="recording"){
    logTimecodeEvent("recording_stop");isRecording=false;
    mediaRecorder.stop();recBtn.textContent="● REC";recBtn.classList.remove("recording");
  }else{
    recordedChunks=[];timecodeLog=[];recordingStartTime=performance.now();isRecording=true;
    logTimecodeEvent("recording_start",{format:getFormatLabel(),canvas:`${canvas.width}x${canvas.height}`});
    const fmt=pickRecordingFormat();
    const fps=isMobile?30:60,bitrate=isMobile?2_000_000:20_000_000;
    const stream=canvas.captureStream(fps);
    mediaRecorder=new MediaRecorder(stream,{mimeType:fmt.mimeType,videoBitsPerSecond:bitrate});
    mediaRecorder.ondataavailable=e=>{if(e.data.size>0)recordedChunks.push(e.data);};
    mediaRecorder.onstop=()=>{
      const blob=new Blob(recordedChunks,{type:fmt.type});const url=URL.createObjectURL(blob);
      const a=document.createElement("a");a.href=url;a.download="space-is-the-face."+fmt.ext;a.click();URL.revokeObjectURL(url);
      if(timecodeLog.length>1){setTimeout(()=>exportTimecodesSRT(),500);setTimeout(()=>exportTimecodesCSV(),1000);}
    };
    mediaRecorder.start(isMobile?1000:undefined);recBtn.textContent="■ STOP";recBtn.classList.add("recording");
  }
};
...
let undoThrottleTimer=null,undoIsRestoring=false;
...
function pushUndo(){
  ...
  undoThrottleTimer=setTimeout(()=>{
    ...
    undoStack.push(snap);if(undoStack.length>UNDO_MAX)undoStack.shift();
  },300);
}

function undo(){if(undoStack.length===0)return;undoIsRestoring=true;const prev=undoStack.pop();setState(prev);undoIsRestoring=false;}

SLIDER_IDS.forEach(id=>{const s=sl(id);if(!s)return;const origOninput=s.oninput;s.oninput=()=>{pushUndo();if(origOninput)origOninput();};});
const origColourOninput=colourPicker.oninput;
colourPicker.oninput=()=>{pushUndo();if(origColourOninput)origColourOninput();};
undoStack.push(getState());

// ─── BPM sync ─────────────────────────────────────────────────────
let bpmValue=120,bpmBeatTime=0,bpmBeatCount=0,bpmInterval=null,bpmDivision=1;
const tapTimes=[],tapBtn=sl("tapTempoBtn"),bpmInput=sl("bpmInput"),beatDisplay=sl("beatDisplay");

if(tapBtn)tapBtn.onclick=()=>{
  const now=performance.now();tapTimes.push(now);if(tapTimes.length>8)tapTimes.shift();
  if(tapTimes.length>=2){let sum=0;for(let i=1;i<tapTimes.length;i++)sum+=(tapTimes[i]-tapTimes[i-1]);const avgMs=sum/(tapTimes.length-1);bpmValue=Math.max(20,Math.min(300,Math.round(60000/avgMs)));if(bpmInput)bpmInput.value=bpmValue;restartBpmTimer();}
  setTimeout(()=>{if(performance.now()-tapTimes[tapTimes.length-1]>3000)tapTimes.length=0;},3500);
};

if(bpmInput)bpmInput.onchange=()=>{bpmValue=Math.max(20,Math.min(300,+bpmInput.value));bpmInput.value=bpmValue;restartBpmTimer();};
...
function restartBpmTimer(){
  ...
  bpmInterval=setInterval(()=>{
    bpmBeatTime=performance.now();bpmBeatCount=(bpmBeatCount%4)+1;
    if(beatDisplay){const dots=['·','·','·','·'];dots[bpmBeatCount-1]='●';beatDisplay.textContent=dots.join(' ');beatDisplay.style.color='#fff';setTimeout(()=>{if(beatDisplay)beatDisplay.style.color='#555';},80);}
...
// ─── BPM auto-detect ──────────────────────────────────────────────
let bpmDetectOnsets=[],bpmDetectLastEnergy=0,bpmDetectLastTime=0;

function detectBPM(){
  if(!sl("bpmDetect")||!sl("bpmDetect").checked)return;
  if(!analyser||!audioFreqData)return;
  const now=performance.now();if(now-bpmDetectLastTime<30)return;bpmDetectLastTime=now;
  analyser.getByteFrequencyData(audioFreqData);
  let energy=0;for(let i=0;i<10;i++)energy+=audioFreqData[i];energy/=10;
  const threshold=bpmDetectLastEnergy*1.4+10;
  if(energy>threshold&&energy>40){if(bpmDetectOnsets.length===0||now-bpmDetectOnsets[bpmDetectOnsets.length-1]>200)bpmDetectOnsets.push(now);if(bpmDetectOnsets.length>16)bpmDetectOnsets.shift();}
  ...
  if(bpmDetectOnsets.length>=4){
    ...
    if(bpmDetectOnsets.length>=4){
      let sum=0;for(let i=1;i<bpmDetectOnsets.length;i++)sum+=(bpmDetectOnsets[i]-bpmDetectOnsets[i-1]);
      const avgMs=sum/(bpmDetectOnsets.length-1);let detected=Math.round(60000/avgMs);
      if(detected>=20&&detected<=300){bpmValue=Math.round(bpmValue*0.7+detected*0.3);if(bpmInput)bpmInput.value=bpmValue;restartBpmTimer();}
    }
  }
}

// ─── Timeline ─────────────────────────────────────────────────────
let timelineCues=[],timelineIndex=-1,timelinePlaying=false,timelineTimer=null;

function fmtTimestamp(s){if(s===null||s===undefined||!isFinite(s))return"--:--";const m=Math.floor(s/60),sec=Math.floor(s%60);return m+":"+String(sec).padStart(2,'0');}
function parseTimestamp(str){if(!str||str==="--:--")return null;const parts=str.split(":");if(parts.length===2)return(+parts[0])*60+(+parts[1]);if(parts.length===1)return+parts[0];return null;}

function renderTimelineCues(){
  const container=sl("timelineCues");if(!container)return;
...
window.addTimelineCue=function(){const morphTime=getMorphTime();let triggerAt=null;if(audioTrackEl&&audioTrackEl.duration)triggerAt=Math.round(audioTrackEl.currentTime);timelineCues.push({state:getState(),transitionTime:morphTime>0?morphTime:3,label:'Cue '+(timelineCues.length+1),triggerAt:triggerAt});sortTimelineCues();renderTimelineCues();updateTimelineStatus();};
...
window.setTimelineCueAt=function(i,val){if(!timelineCues[i])return;const t=parseTimestamp(val);timelineCues[i].triggerAt=t;sortTimelineCues();renderTimelineCues();};
window.removeTimelineCue=function(i){timelineCues.splice(i,1);renderTimelineCues();updateTimelineStatus();};
window.moveTimelineCue=function(i,dir){const j=i+dir;if(j<0||j>=timelineCues.length)return;[timelineCues[i],timelineCues[j]]=[timelineCues[j],timelineCues[i]];renderTimelineCues();};

function sortTimelineCues(){const allHaveTime=timelineCues.every(c=>c.triggerAt!==null&&c.triggerAt!==undefined);if(allHaveTime&&timelineCues.length>1)timelineCues.sort((a,b)=>a.triggerAt-b.triggerAt);}

window.playTimeline=function(){
  if(timelineCues.length===0)return;timelinePlaying=true;timelineIndex=-1;
  const hasTimestamps=timelineCues.some(c=>c.triggerAt!==null&&c.triggerAt!==undefined);
  if(hasTimestamps&&audioTrackEl&&audioTrackEl.duration){if(!audioTrackPlaying){audioTrackEl.currentTime=0;audioTrackEl.play();audioTrackPlaying=true;const btn=sl("audioPlayBtn");if(btn)btn.textContent="❚❚";}}
  else{timelineIndex=0;morphToCue(0);}
  updateTimelineStatus();
};

window.stopTimeline=function(){timelinePlaying=false;if(timelineTimer)clearTimeout(timelineTimer);timelineTimer=null;updateTimelineStatus();renderTimelineCues();};
window.nextTimelineCue=function(){if(timelineCues.length===0)return;timelineIndex=(timelineIndex+1)%timelineCues.length;morphToCue(timelineIndex);renderTimelineCues();updateTimelineStatus();};

function tickTimelineSync(){
  if(!timelinePlaying)return;if(!audioTrackEl||!audioTrackPlaying)return;
  const pos=audioTrackEl.currentTime;
  const hasTimestamps=timelineCues.some(c=>c.triggerAt!==null&&c.triggerAt!==undefined);if(!hasTimestamps)return;
  let targetIndex=-1;
  for(let i=0;i<timelineCues.length;i++){if(timelineCues[i].triggerAt!==null&&timelineCues[i].triggerAt!==undefined&&pos>=timelineCues[i].triggerAt)targetIndex=i;}
  if(targetIndex>=0&&targetIndex!==timelineIndex)morphToCue(targetIndex);
}

function morphToCue(i){
  const cue=timelineCues[i];if(!cue)return;timelineIndex=i;renderTimelineCues();
  ...
  if(dur<=0){setState(cue.state);}else{
    morphStartState={};SLIDER_IDS.forEach(id=>{const s=sl(id);if(s)morphStartState[id]=+s.value;});
    morphStartState.colour=colourPicker.value;morphStartState.trackballs={};
    Object.keys(trackballState).forEach(k=>{morphStartState.trackballs[k]={x:trackballState[k].x,y:trackballState[k].y};});
    morphTargets={...cue.state};if(cue.state.trackballs)morphTargets.trackballs={...cue.state.trackballs};
    morphDuration=dur*1000;morphStart=performance.now();
  }
  const hasTimestamps=timelineCues.some(c=>c.triggerAt!==null&&c.triggerAt!==undefined);
  if(timelinePlaying&&!hasTimestamps){
    if(timelineTimer)clearTimeout(timelineTimer);
    const nextI=(i+1)%timelineCues.length;const waitMs=dur*1000+500;
    timelineTimer=setTimeout(()=>{if(!timelinePlaying)return;if(nextI===0){timelinePlaying=false;updateTimelineStatus();renderTimelineCues();}else{morphToCue(nextI);}},waitMs);
  }
}

function updateTimelineStatus(){
  const el=sl("timelineStatus");if(!el)return;
  if(timelinePlaying){const hasTs=timelineCues.some(c=>c.triggerAt!==null&&c.triggerAt!==undefined);el.textContent=hasTs?`▶ Synced to audio — cue ${timelineIndex+1}/${timelineCues.length}`:`▶ Playing cue ${timelineIndex+1}/${timelineCues.length}`;}
...
sl("timelineImportFile").onchange=e=>{
  ...
  r.onload=()=>{
    try{
      ...
      if(Array.isArray(data)){if(data.length>0&&data[0].state){timelineCues=data.map((c,i)=>({state:c.state,transitionTime:c.transitionTime||3,label:c.label||'Cue '+(i+1),triggerAt:c.triggerAt!==undefined?c.triggerAt:null}));}}
      else if(typeof data==='object'&&data.colour){let triggerAt=null;if(audioTrackEl&&audioTrackEl.duration)triggerAt=Math.round(audioTrackEl.currentTime);timelineCues.push({state:data,transitionTime:3,label:'Imported',triggerAt:triggerAt});}
      renderTimelineCues();updateTimelineStatus();
...
function renderLoop(){
  ...
  lastTimestamp=now;
  tickMorph();updateAudioWaveform();detectBPM();updateAudioTrackUI();tickTimelineSync();updateVideoTransportUI();

  const videoReady=video.readyState>=2&&!video.paused&&!video.ended;
  let faceLandmarksThisFrame=null;
  if(videoReady){try{const result=faceLandmarker.detectForVideo(video,now);if(result.faceLandmarks&&result.faceLandmarks.length>0)faceLandmarksThisFrame=result.faceLandmarks[0];}catch(e){}}

  if(faceLandmarksThisFrame){
    currentLandmarks=faceLandmarksThisFrame;renderDepth(currentLandmarks);
    let grid=buildGrid(currentLandmarks);grid=smoothGrid(grid);lastGrid=grid;
    render(currentLandmarks,grid);statusEl.textContent="";
  }else if(currentLandmarks&&lastGrid){
    render(currentLandmarks,lastGrid);
    if(videoFileActive&&video.paused)statusEl.textContent="⏸ Paused";else if(videoFileActive)statusEl.textContent="";else statusEl.textContent="No face detected";
  }else{
    const grid=prevGrid||Array.from({length:NUM_LINES},()=>new Float32Array(LINE_SAMPLES));lastGrid=grid;
    const safeAmt=sl("safeAreaSlider")?+sl("safeAreaSlider").value:0;const safeFlip=sl("safeFlipCheck")?sl("safeFlipCheck").checked:false;
    ctx.fillStyle="#000";ctx.fillRect(0,0,canvas.width,canvas.height);
    ctx.save();ctx.translate(canvas.width,0);ctx.scale(-1,1);drawLines(grid,safeAmt,safeFlip);ctx.restore();
    drawWebcamOverlay();statusEl.textContent="No face detected";
  }
