...
// ─── Render loop ───────────────────────────────────────────────────
function renderLoop(){
  if(!faceLandmarker){requestAnimationFrame(renderLoop);retur// main.js — sPACE iS tHE faCE
import vision from "https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@0.10.3";
import { TRIANGULATION } from "./triangulation.js";
const { FaceLandmarker, FilesetResolver } = vision;
...
sl("audioFileInput").onchange=async e=>{
  ...
  audioTrackEl=new Audio();
  audioTrackEl.src=URL.createObjectURL(f);
  audioTrackEl.preload="auto";

  // Connect to Web Audio for analyser
  ...
  audioTrackMediaSource.connect(audioTrackAnalyser);
  audioTrackAnalyser.connect(audioCtx.destination);

  // Route to audio reactive if checked
...
// Re-route analyser when checkbox toggled
...
// Seek bar
sl("audioTrackSeek").oninput=function(){
  if(!audioTrackEl||!audioTrackDuration)return;
  audioTrackEl.currentTime=(+this.value/1000)*audioTrackDuration;
};
...
function updateAudioTrackUI(){
  ...
  const seekEl=sl("audioTrackSeek");
  if(seekEl&&audioTrackDuration)seekEl.value=Math.round((pos/audioTrackDuration)*1000);
  const timeEl=sl("audioTrackTime");
  if(timeEl)timeEl.textContent=`${fmtTime(pos)} / ${fmtTime(audioTrackDuration)}`;
}
